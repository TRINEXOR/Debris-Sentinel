...
def _load_model():
    ...
    try:
        ...
        # Disable ensemble on free tier to avoid loading multiple 57MB checkpoints
        predictor.ensemble_paths = []
...
async def predict(request: PredictRequest, req: Request):
    ...
    try:
        ...
        log.exception("OOM during prediction")
        raise HTTPException(status_code=503, detail="Server ran out of memory. Try again in a moment.")
    except Exception as e:
...
if _FRONTEND_DIST.exists():
    ...
    # Serve index.html only for true frontend routes (not API paths)
    @app.get("/")
    ...
    async def serve_frontend(full_path: str):
        """Serve frontend SPA — skip API paths, return index.html for all others."""
        # Don't intercept API or known backend routes
        api_prefixes = ("predict", "satellites", "satellite", "health", "maneuver", "interpret", "docs", "redoc", "openapi")
